...
def test_super_user_is_not_superuser(user_factory):
    """Test that an error is raised when an admin super user has is_superuser set to False"""
    with pytest.raises(ValueError) as err:
...
def test_create_user_with_no_firstname(user_factory):
    """Test creating a new user without a firstname raises ab error"""
    with pytest.raises(ValueError) as err:
...
def test_create_user_with_no_lastname(user_factory):
    """Test creating a new user without a lastname raises ab error"""
    with pytest.raises(ValueError) as err:
...
def test_user_email_incorrect(user_factory):
    ...
    with pytest.raises(ValueError) as err:
        user_factory.create(email="kitetutu.com")
